...
INTER_CA_DOCKER=cmb-inter-ca
ORG_BASE=cmb
SERVER_CA_PORT=7058

CA_USER=admin
CA_PASSWD=adminpw
CA_CLIENT=`pwd`/../bin/fabric-ca-client
NUM_ORGS=1
NUM_PEERS_PER_ORG=2
...
mkdir admincerts cacerts intermediatecerts keystore signcerts
ORG_ADMIN_ROOT=`pwd`
cd $CA_ROOT && $CA_CLIENT enroll -u http://$CA_USER:$CA_PASSWD@localhost:$SERVER_CA_PORT -c fabric-ca-client/fabric-ca-client-config.yaml --id.name Admin.${ORG_BASE}.ca
cd $ORG_ADMIN_ROOT
mv $CA_ROOT/fabric-ca-client/msp/keystore/*_sk keystore/admin.${ORG_BASE}.key
...
cp $ORG_CA_ROOT/ca/ca.${ORG_BASE}.cert.pem intermediatecerts/
...
mkdir admincerts cacerts intermediatecerts keystore signcerts
_PWD=`pwd`
cd $CA_ROOT && $CA_CLIENT enroll -u http://$CA_USER:$CA_PASSWD@localhost:$SERVER_CA_PORT -c fabric-ca-client/fabric-ca-client-config.yaml 
cd $_PWD
mv $CA_ROOT/fabric-ca-client/msp/keystore/*_sk keystore/orderer.${ORG_BASE}.key
...
function generatePeer() {
	...
	cd $CA_ROOT && $CA_CLIENT enroll -u http://$CA_USER:$CA_PASSWD@localhost:$SERVER_CA_PORT -c fabric-ca-client/fabric-ca-client-config.yaml 
	cd $_PWD
	mv $CA_ROOT/fabric-ca-client/msp/keystore/*_sk keystore/peer${PEER_ID}.org${ORG_ID}.${ORG_BASE}.key
	mv $CA_ROOT/fabric-ca-client/msp/signcerts/cert.pem signcerts/peer${PEER_ID}.org${ORG_ID}.${ORG_BASE}.cert.pem
	cp $ORG_CA_ROOT/ca/ca.org${ORG_ID}.${ORG_BASE}.cert.pem intermediatecerts/
...
function generateOrg() {
	...
	cd $CA_ROOT && $CA_CLIENT enroll -u http://$CA_USER:$CA_PASSWD@localhost:$SERVER_CA_PORT -c fabric-ca-client/fabric-ca-client-config.yaml 
	cd $ORG_ADMIN_ROOT
	mv $CA_ROOT/fabric-ca-client/msp/keystore/*_sk keystore/admin.org${ORG_ID}.${ORG_BASE}.key
