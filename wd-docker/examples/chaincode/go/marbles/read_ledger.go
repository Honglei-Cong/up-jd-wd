...
func read_everything(stub shim.ChaincodeStubInterface) pb.Response {
	...
	for resultsIterator.HasNext() {
		...
		fmt.Println("on marble id - ", kv.Key)
		var marble Marble
		json.Unmarshal(kv.Value, &marble)                  //un stringify it aka JSON.parse()
	...
	for ownersIterator.HasNext() {
		...
		kv, err := ownersIterator.Next()
		if err != nil {
			return shim.Error(err.Error())
		}
		
		fmt.Println("on owner id - ", kv.Key)
		var owner Owner
		json.Unmarshal(kv.Value, &owner)                  //un stringify it aka JSON.parse()
...
func getHistory(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	for resultsIterator.HasNext() {
		...
		km, err := resultsIterator.Next()
		if err != nil {
			return shim.Error(err.Error())
		}

		var tx AuditHistory
		tx.TxId = km.TxID                             //copy transaction id over
		json.Unmarshal(km.Value, &marble)     //un stringify it aka JSON.parse()
		if km.Value == nil {                  //marble has been deleted
